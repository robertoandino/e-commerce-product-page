...
const Automobiles = () => {
    ...
    return (
        <div className="container mx-auto p-8 mt-40 max-w-3xl bg-gray-900 bg-opacity-60 rounded-2xl shadow-xl text-white">
            {images.map((image, index) => (
                <div key={index} className="flex flex-col items-center mb-16 transition-tranform duration-500 ease-in-out">
                    <h2 className="text-3xl font-serif text-yellow-300 mb-4">{descriptions[index].title}</h2>
                    <p className="text-yellow-400 text-lg mb-2">{descriptions[index].info}</p>
            ...
            <div className="text-center mt-10">
                <h3 className="text-2xl mb-4">Interested? Contact Us</h3>
